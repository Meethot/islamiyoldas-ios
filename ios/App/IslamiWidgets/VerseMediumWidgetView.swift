...
// MARK: - Medium Verse Widget View

struct VerseMediumWidgetView: View {
...
// MARK: - Mihrab Arch Shape (Islamic Arch)

struct MihrabArchShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let w = rect.width
        let h = rect.height
        let archHeight = h * 0.45
        
        // Left pillar
        path.move(to: CGPoint(x: 0, y: h))
        path.addLine(to: CGPoint(x: 0, y: archHeight))
        
        // Arch (pointed Islamic arch)
        path.addQuadCurve(
            to: CGPoint(x: w / 2, y: 0),
            control: CGPoint(x: 0, y: archHeight * 0.2)
        )
        path.addQuadCurve(
            to: CGPoint(x: w, y: archHeight),
            control: CGPoint(x: w, y: archHeight * 0.2)
        )
